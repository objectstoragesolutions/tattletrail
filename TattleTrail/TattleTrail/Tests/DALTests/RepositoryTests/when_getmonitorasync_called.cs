...
namespace TattleTrail.Tests.DALTests.RepositoryTests {
    ...
    public class when_getmonitorasync_called {
        Establish _context = () => {
        ...
        Because of = async () =>
            await repository.GetMonitorAsync(monitor.Id);

        It should_call_hashgetallasync_once = () =>
            Mock.Get(provider).Verify(x => x.Database.HashGetAllAsync(monitor.Id.ToString(), CommandFlags.None), Times.Once);
